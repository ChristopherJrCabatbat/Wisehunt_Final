...
function toggleRedDotVisibility() {
    ...
    const hasNotifications =
        notificationList.querySelector(".notification-item") !== null;
...
function addLowQuantityNotifications() {
    ...
    lowQuantityNotifications.forEach(function (product) {
        ...
        if (product.forecastMessage && product.forecastMessage.trim() !== "") {
            const forecastMessageItem = document.createElement("li");
            forecastMessageItem.classList.add(
                "notification-item",
                "forecast-message"
            );
            forecastMessageItem.innerHTML = `
...
//     window.location.href = `{{ route('admin.productEdit', ['id' => '__productId__']) }}`.replace('__productId__', productId);
...
function navigateToProductView(productId) {
    ...
    // setTimeout(function () {
    //     var route = document.getElementById("notificationPanel").getAttribute("data-route");
    //     window.location.href = route.replace("__productId__", productId);
    // },);
}
